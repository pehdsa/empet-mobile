...
interface SightingListProps {
  ...
  onOpenFilters: () => void;
  onItemPress: (sighting: PetSighting) => void;
...
function buildLocationText(sighting: PetSighting): string | null {
  if (sighting.distanceMeters != null) {
    return `${formatDistance(sighting.distanceMeters)}${sighting.addressHint ? ` - ${sighting.addressHint}` : ""}`;
  }
  return sighting.addressHint ?? null;
}
    ...
    <FlatList
      ...
      renderItem={({ item }) => (
        <PetGridCard
          ...
          title={item.title}
          subtitle={`${speciesLabel[item.species]}${item.size ? ` · ${sizeLabel[item.size]}` : ""}${item.color ? ` · ${item.color}` : ""}`}
          locationText={buildLocationText(item)}
          timeText={getDaysAgo(item.sightedAt)}
